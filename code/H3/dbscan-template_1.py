...
def main():
	...
	with open('.'+os.sep+'Output_2'+os.sep+'labels_' + str(sys.argv[2]) + '_'  + str(sys.argv[3]) + '.txt', "w") as f:
		for e in db[0]:
			f.write(str(e))
			f.write('\n')

	# store output core sample indexes returned by your algorithm for automatic marking
	with open('.'+os.sep+'Output_2'+os.sep+'core_sample_indexes_'  + str(sys.argv[2]) + '_'+ str(sys.argv[3]) + '.log', "w") as f:
		for e in db[1]:
			f.write(str(e))
			f.write('\n')
	...
	if dimension == 2:
		...
		plt.savefig('.'+os.sep+'Output_2'+os.sep+'cluster-result_'  + str(sys.argv[2]) + '_' + str(sys.argv[3]) +'.png')
